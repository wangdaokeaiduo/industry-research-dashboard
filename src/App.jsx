...
function DecisionActions({ catalysts=[], execution={}, bearCase=[], evidence=[] }) {
  return <div className="action-system">{catalysts.length>0&&<div className="catalyst-list"><h3>催化剂日历</h3>{catalysts.map(item=><div key={`${item.date}-${item.event}`}><time>{item.date}</time><p><strong>{item.event}</strong><span>{item.expectation}</span></p><em className={item.status}>{item.statusLabel||'待验证'}</em></div>)}</div>}<div className="execution-card"><small>当前动作</small><strong>{execution.action}</strong>{[['首次触发',execution.trigger],['加仓确认',execution.add],['价格失效',execution.priceInvalidation],['基本面失效',execution.fundamentalInvalidation],['下次复核',execution.nextReview]].filter(([,v])=>v).map(([k,v])=><div key={k}><b>{k}</b><span>{v}</span></div>)}</div>{(bearCase.length>0||evidence.length>0)&&<div className="bear-evidence">{bearCase.length>0&&<div><h3>空头辩护</h3>{bearCase.map((item,i)=><p key={item}><b>0{i+1}</b>{item}</p>)}</div>}{evidence.length>0&&<div><h3>证据质量</h3>{evidence.map(item=><p key={item.label}><b className={`grade grade-${item.grade}`}>{item.grade}</b><span>{item.label}</span><small>{item.note}</small></p>)}</div>}</div>}</div>
}
...
function StockTechnicalDetail({ data={} }) {
  ...
  return <div className="stock-technical-detail">
    ...
    <div className="stock-tech-block signal-block"><h3>五、综合研判</h3><div><section className="bull-signals"><strong>多头信号（占优）</strong>{detail.signals?.bull?.map((item,i)=><p key={item}><b>0{i+1}</b>{item}</p>)}</section><section className="risk-signals"><strong>风险信号</strong>{detail.signals?.risk?.map((item,i)=><p key={item}><b>0{i+1}</b>{item}</p>)}</section></div><blockquote><small>技术面结论</small><strong>{data.rating||detail.headline}</strong><p>{detail.plainConclusion}</p><dl><div><dt>介入触发</dt><dd>{data.trigger||'等待价格与量能共同确认'}</dd></div><div><dt>结构失效</dt><dd>{data.invalidation||'跌破关键支撑'}</dd></div></dl></blockquote></div>
  </div>
}

function Technical({ data={} }) {
  const deepStock=Boolean(data.technicalDetail)
  ...
  return <><StockTechnicalDetail data={data}/>{!deepStock&&<PlainTechnicalReadout data={data}/>}<div className={deepStock?'stock-chart-section':''}>{deepStock&&<div className="stock-chart-title"><small>真实行情 · 最近50个交易日</small><h3>六、K线、量能与交易验证</h3><p>悬停查看OHLC和成交额，拖动下方手柄调整观察区间；触发位与失效位直接标注在价格轴上。</p></div>}<div className="technical-grid"><div className="chart-panel"><div className="chart-toolbar"><div><strong>{data.instrument}</strong><span>{data.timeframe}{data.ohlc?.length?'':' · 收盘线'}</span></div><div className="ma-legend">{[[5,'#e85d3f'],[10,'#d99a2b'],[20,'#8c63c7'],[60,'#2f80c9'],[90,'#25a59a'],[145,'#586473']].map(([period,color])=><span key={period} style={{color}}><i/>MA{period}</span>)}</div></div><PriceChart prices={data.prices} volumes={data.volumes} ohlc={data.ohlc} levels={levels}/></div>
    ...
    </dl><div className="trade-level"><span><small>触发条件</small>{data.trigger}</span><span><small>失效条件</small>{data.invalidation}</span></div></div></div><VolumeMatrix data={data}/></div></>
}
